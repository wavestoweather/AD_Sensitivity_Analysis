...
if __name__ == "__main__":
    parser = ArgumentParser(description=
            '''Load data given a prefix and suffix of the filenames and
            plot various figures, mostly heatmaps.''')
    parser.add_argument("-p", "--prefix", type=str, required=True,
            help='''Prefix of the files to load such as "data/sb_ice_traj".''')
    parser.add_argument("-s", "--suffix", type=str, required=True,
            help='''Suffix of the files to load without datatype such as
            "_start_over_20160922_00".''')
    parser.add_argument("-i", "--image", type=str, default="results",
            help='''Which plot to generate. Can be either "results" or
            "derivatives" to plot lineplots
    ...
    parser.add_argument("-f", "--filter", action="store_true", default=True,
            help='''If you load derivatives, use this to filter derivatives
    ...
    if args.image == "results":
        df_results = load_output(filename=args.prefix + args.suffix + ".txt")
        plot_line_res(df=df_results, prefix=args.suffix)
        ...
        plot_line(df_deriv, prefix=args.suffix)
