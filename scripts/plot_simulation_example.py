...
def load_data(
    ...
    ds = xr.open_dataset(file_list_cosmo[0], decode_times=False)
    cols = list(ds.keys())
    ...
    for f in file_list:
        if verbosity > 0:
            print(f"Loading {f}")
        ds = xr.open_dataset(f, decode_times=False)[cols_final_sim]
    ...
    for f in file_list_cosmo:
        if verbosity > 0:
            print(f"Loading {f}")
        ds = xr.open_dataset(f, decode_times=False)[cols_final]
