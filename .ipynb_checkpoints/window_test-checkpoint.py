...
t = timer()
df.assign(conv_400=False)
# Could also do 0 to 3000
# Need to check for additional flags...
# Perhaps enumerate all trajectories (would be easier)
i = 0
...
#     t = timer()
    df = df.merge(data_st, "outer")
    if i == 5:
        break
#     t2 = timer()
#     print("Merging done in {} s".format(t2-t))
t2 = timer()
print("Conv done in {} s".format(t2-t))
print("Starting plot")
xticks = np.arange(0, 200000, window_conv_400)
t = timer()
img = df.hvplot.scatter(x="timestep", y="p", by="conv_400", width=1600, height=900, datashade=True, xticks=xticks).opts(xformatter="%2.2e", yformatter="%2.2e")
t2 = timer()
print("Plotting done in {} s".format(t2-t))
# hv.save(img, "scatter.png")
# img
    # Output so far
...
renderer = hv.Store.renderers['matplotlib'].instance(
                    fig='png', dpi=300)
renderer.save(img, "pics/scatter.png")