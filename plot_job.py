...
#SBATCH --job-name=TrajHexNSInputs
#SBATCH --output=logs/TrajHexNSInputs_%A.out
#SBATCH --error=logs/TrajHexNSInputs_%A.err
#SBATCH --mem=120GB
...
print("Using {} cpus".format(ncpus), flush=True)
out_path = "/lustre/project/m2_zdvresearch/mahieron/pics"

for key in keys:
    print("Plotting for {}".format(key), flush=True)
    ...
    t2 = timer()
    print("Loading done in {}s".format(t2-t), flush=True)
    ...
    data.plot_two_ds(
        ...
        sort=sort,
        errorband=False,
        plot_path=out_path + "/test13_ds_" + key + "_",
        fig_type="png",
        **kwargs
    )
    t2 = timer()
    print("Plot two (p) done in {}s\n".format(t2-t), flush=True)
...
for key in keys:
    ...
    data.plot_two_ds(
        ...
        sort=sort,
        errorband=False,
        plot_path=out_path + "/test14_ds_" + key + "_",
        fig_type="png",
        **kwargs
    )
    t2 = timer()
